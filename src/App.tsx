...
import { FilterState, MusicalEvent } from './types';
...
const App: React.FC = () => {
  ...
  // Generate events once on mount/render (pure function)
  const allEvents = useMemo(() => generateAllEvents(), []);
  ...
  return (
    <div className="min-h-screen bg-[#f1f2f6] text-[#033d60] pb-20">
      ...
      {/* Header */}
      <header className="bg-white border-b border-slate-200 py-4 px-4 md:px-8 shadow-sm sticky top-0 z-40">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <img 
            src="/assets/logo.png" 
            alt="Setor Musical - Regional Uruguaiana e Fronteira" 
          ...
          <nav className="flex gap-3">
            ...
            <button
              onClick={() => setCurrentView('churches')}
              className={`px-4 py-2 rounded-lg font-medium text-sm flex items-center gap-2 transition-colors ${
                currentView === 'churches'
                  ? 'bg-[#033d60] text-white'
                  : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
              }`}
            >
